    ...
    <div className="auth-wrap">
      <div className="auth-card">
        ...
        <div className="auth-sub">
          ...
          {view === "forgot" && "Reset your secure password."}
        </div>
        
        {error && <div style={{ background: "#fdecea", color: "var(--red)", padding: "10px", borderRadius: "8px", fontSize: "0.8rem", marginBottom: "12px" }}>{error}</div>}
        {msg && <div style={{ background: "var(--green-pale)", color: "var(--green)", padding: "10px", borderRadius: "8px", fontSize: "0.8rem", marginBottom: "12px" }}>{msg}</div>}

        {/* ── GOOGLE BUTTON (Only on Login/Signup) ── */}
        {view !== "forgot" && (
          <>
            <button className="btn btn-outline" style={{ width: "100%", justifyContent: "center", marginBottom: "16px", color: "var(--text)" }} onClick={handleGoogleLogin} disabled={loading}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "10px", margin: "16px 0", color: "var(--muted)", fontSize: "0.75rem", textTransform: "uppercase" }}>
              <hr style={{ flex: 1, border: "none", borderTop: "1px solid var(--border)" }} /> OR <hr style={{ flex: 1, border: "none", borderTop: "1px solid var(--border)" }} />
            </div>
          </>
        )}

        {/* ── EMAIL/PASSWORD FORM ── */}
        <form className="auth-form" onSubmit={view === "forgot" ? handleResetPassword : handleEmailAuth}>
          <input type="email" className="input" placeholder="Email address" value={email} onChange={e => setEmail(e.target.value)} required />
          
          {view !== "forgot" && (
            <input type="password" className="input" placeholder="Password (min 6 chars)" value={password} onChange={e => setPassword(e.target.value)} required minLength={6} />
          )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 16 }}>
          {view === "login" && <button className="auth-toggle" style={{ marginTop: 0 }} onClick={() => { setView("forgot"); setError(null); setMsg(null); }}>Forgot your password?</button>}
          
          <button className="auth-toggle" style={{ marginTop: 0 }} onClick={() => { setView(view === "login" ? "signup" : "login"); setError(null); setMsg(null); }}>
